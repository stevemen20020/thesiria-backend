...
export class racesController {
    ...
    getRaceses = async (req:Request, res:Response, next:NextFunction) => {
        try {
            ...
            const searchQueryParams = racesDtoValidator.validateSearchRacesParamsQueryDto(req.query)

            const [data, total] = await this.repository.getRaceses(
                searchQueryParams
            )
    ...
    getRacesById = async (req:Request, res:Response, next:NextFunction) => {
        try {
            ...
            const data = await this.repository.getRacesById(
                String(id)
            )
    ...
    updateRaces = async (req:Request, res:Response, next:NextFunction) => {
        try {
            ...
            const data = await this.repository.updateRaces(
                updateBody,
                String(id)
            )
    ...
    deleteRaces = async (req:Request, res:Response, next:NextFunction) => {
        try {
            ...
            const data = await this.repository.deleteRaces(
                String(id)
            )
